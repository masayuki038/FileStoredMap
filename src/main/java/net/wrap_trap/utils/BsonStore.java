...
public class BsonStore<V> implements Store<V> {
    ...
    protected BSONObject readFrom(String key, RandomAccessFile indexFile, Position indexRef) throws IOException,
        ...
        if (dataRef == null)
            return null;
        return readFrom(key, dataRef.getPointer(), dataRef.getFileNumber());
    }
    ...
    protected BSONObject readDataFile(String key, int dataPos, byte fileNumber) throws IOException {
        ...
        DataBlock dataRef = getDataRef(dataPos, fileNumber);
    ...
    protected DataBlock getDataRef(int dataPos, byte fileNumber) throws IOException {
    ...
    protected BSONObject removeBSON(String key, int dataPos, byte dataFileNumber, RandomAccessFile indexFile,
        ...
        DataBlock dataRef = getDataRef(dataPos, dataFileNumber);
        if (key.equals(getKey(dataRef.getBsonObject()))) {
            if ((dataRef.getNextPointer() == 0) && (dataRef.getNextFileNumber() == 0) && (dataRefList.size() == 0)) {
                ...
                indexer.updateIndex(indexFile, indexPos, dataRef.getNextPointer(), dataRef.getNextFileNumber());
            }
